...
const navLinks = ['Dashboard', 'Projects', 'Team'];

function Layout({ children }: React.PropsWithChildren<{}>) {
  const { colorMode, toggleColorMode } = useColorMode();

  return (
    <Box minH="100vh" bg={useColorModeValue('gray.100', 'gray.900')} px={4}>
      <Box py="6" borderBottom="1px" borderColor="gray.300">
        <Container maxW="container.lg" display="flex" justifyContent="space-between" alignItems="center">
          ...
          <HStack aria-label="Main navigation" spacing={8} alignItems={'center'}>
            <HStack
              as={'nav'}
              spacing={4}
      ...
      <Flex justifyContent="space-between" alignItems="flex-start" flex="1">
        <Container maxW="container.lg" display="flex" justifyContent="space-between" alignItems="flex-start" flex="1">{children}</Container>
      </Flex>
      <Box py="6" borderTop="1px" borderColor="gray.300">
        <HStack aria-label="Main footer" spacing={8} justify="center" alignItems={'center'}>
          <HStack
            spacing={4}
            display={{ base: 'none', md: 'flex' }}>
            <Text as="p">&copy; You!</Text>
          </HStack>
        </HStack>
      </Box>
    </Box>
...
export function ErrorBoundary({ error }: { error: Error }) {
  ...
  return (
    <Document title="Error!">
      <Layout>
        <Box>
          <Text as="h1">There was an error</Text>
